...
export const AnimeGridBlock = () => {
    ...
    const AddRatingToQuery = useAppSelector(
        (state) => state.ratingFilter.ratingQuery
    )
    const AddSeasonsToQuery = useAppSelector(
        (state) => state.seasonsFilter.seasonQuery
    )
    const AddProducersToQuery = useAppSelector(
        (state) => state.producerFilter.producersQuery
    )
    const AddSearchToQuery = useAppSelector(
        (state) => state.searchFilter.searchQuery
    )
    const AddStatusToQuery = useAppSelector(
        (state) => state.statusFilter.statusType
    )
    const AddSortByToQuery = useAppSelector(
        (state) => state.sortFilter.sortType
    )
    const AddOrderByToQuery = useAppSelector(
        (state) => state.orderByFilter.orderBy
    )
    ...
    const {
        ...
        sfw:
            AddRatingToQuery === 'RX' ||
            AddGenreToQuery.genresName.includes('Hentai')
                ? ''
                : 'true',
    })
