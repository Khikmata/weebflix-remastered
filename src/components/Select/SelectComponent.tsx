...
export const SelectComponent: React.FC<SelectComponentProps> = ({
    ...
    const ratingDisplay = useAppSelector(
        (state) => state.ratingFilter.ratingDisplay
    )
    const producersDisplay = useAppSelector(
        (state) => state.producerFilter.producersDisplay
    )
    const seasonDisplay =
        `${seasonYear} ` + TranslateSeasonToRussian(seasonSeason)
    const statusDisplay = useAppSelector(
    ...
    const displayHandler = useMemo(() => {
        ...
        if (dropDownType === DropDownTypeEnum.RATING) {
            return ratingDisplay
                ? TranslateRatingToRussian(ratingDisplay)
                : tooltip
        }
