    ...
    const getSeasonsDropdown = useMemo(() => {
        const handleProducerChange = (
            ...
            if (index === selectedProducer) {
                dispatch(
                    producersFilterActions.removeProducer(
                        producerData[index].titles[0].title
                    )
                )
                setSelectedProducerIndex(null)
            } else {
                dispatch(
                    producersFilterActions.setProducer(producerData[index])
                )
        ...
        return producerData.map((producer, index) => (
            <li
                key={index}
                onClick={() =>
                    handleProducerChange(index, selectedProducerIndex)
                }
                className={
                    styles[selectedProducerIndex === index ? 'active' : '']
                }
            >
