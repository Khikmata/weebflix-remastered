...
export const GenresDropdown = () => {
  ...
  const selectedGenresIndexes = useAppSelector((state) => state.filterReducer.genreFilters.selectedGenresIndexes)
  console.log(selectedGenresIndexes)
  // Список жанров
  ...
  const getGenreDropdown = useMemo(() => {
    const handleGenreChange = (index: number) => {
      ...
      if (selectedGenresIndexes.includes(index)) {
        dispatch(genreFilterActions.removeGenre(sortedGenres.find((genre) => genre.mal_id === index)!))
      }
      //Отправка конкретеного жанра в стор и добавление в локальный массив
      else {
        dispatch(genreFilterActions.setGenre(sortedGenres.find((genre) => genre.mal_id === index)!))
      }
    }
    ...
    return sortedGenres.map((genre) => (
      <li
        ...
        className={styles[selectedGenresIndexes.includes(genre.mal_id) ? 'active' : '']}
      >
